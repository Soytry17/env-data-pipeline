...
class WeatherValidator:
    """
    Validates raw weather rows BEFORE transformation.
    Each rule is one method — returns a boolean mask of valid rows.
    ...
    # max age of a reading in minutes — reject stale data
    MAX_AGE_MINUTES = 120
    ...
    def validate(self, rows):
        ...
        # track which rows are valid — start with all True
        valid_mask = pd.Series([True] * len(df), index=df.index)

        # run each rule — each one can flip rows to False
    ...
    def _rule_required_fields(self, df, mask):
        """All required fields must exist as columns"""
        ...
        if missing_cols:
            ...
            # fail all rows if schema is broken
            return pd.Series([False] * len(df), index=df.index)
        return mask

    def _rule_no_empty_location(self, df, mask):
        """Location must not be null or empty"""
        bad = df["location"].isna() | (df["location"].astype(str).str.strip() == "")
        if bad.any():
            self._reject(df, bad, "Empty location")
            mask = mask & ~bad
        return mask

    def _rule_numeric_types(self, df, mask):
        """Core measurement columns must be numeric"""
    ...
    def _rule_no_future_timestamps(self, df, mask):
        """extracted_at must not be in the future"""
    ...
    def _reject(self, df, bad_mask, reason):
        """Log rejected rows with their reason"""
